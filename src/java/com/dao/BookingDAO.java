...
public class BookingDAO {
    ...
    public boolean saveBooking(Booking c) {
        boolean f = false;
        try {
            String sql = "INSERT INTO booking(name, email, address, phno, start, end, about, amount, userid, status, driver_id, booking_date) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)";
    ...
    public boolean updateBooking(Booking c) {
        boolean f = false;
        try {
            String sql = "UPDATE booking SET name=?, email=?, address=?, phno=?, start=?, end=?, about=?, amount=?, status=?, driver_id=?, booking_date=? WHERE id=?";
            PreparedStatement ps = conn.prepareStatement(sql);
            ps.setString(1, c.getName());
            ps.setString(2, c.getEmail());
            ps.setString(3, c.getAddress());
